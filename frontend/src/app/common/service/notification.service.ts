...
import { isUserNotification, UserNotification, UserNotificationTopic } from '../types/notification';
import { Client, StompHeaders } from '@stomp/stompjs';
import { Course, isCourse } from '../types/course';
import { isSubscriptionStatus, SubscriptionStatus } from 'src/app/features/subscription/types/subscription';
...
export class UserNotificationService {
  ...
  private readonly _subscriptionStatusSubject = new BehaviorSubject<SubscriptionStatus | null>(null);
  private client?: Client
  private globalClient?: Client
  constructor(private tokenService : ApiTokenService, private sessionService : SessionService) {
  ...
  private connectUserNotification() {
    ...
    console.log("stating stomp connection")
      let client = new Client({
        brokerURL: `ws://${this.apiHost}/notification`,
        onConnect: () => {
  ...
  private publishMessageToSubjects(message : any){
      if(!isUserNotification(message)){
        return;
      }
      let notification = message as UserNotification;
      if(notification.topic == UserNotificationTopic.COURSE_UPDATE && isCourse(notification.content)){
          this._courseSubject.next(notification.content)
      } else if(notification.topic == UserNotificationTopic.SUBSCRIPTION_UPDATE && isSubscriptionStatus(notification.content)){
